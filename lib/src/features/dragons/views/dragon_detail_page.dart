...
class DragonDetailPage extends StatelessWidget {
  const DragonDetailPage({Key? key, required this.dragon}) : super(key: key);
...
class _BackgroundImage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (canShowImage) {
      ...
      return Image.network(dragon.flickrImages!.first);

      print(dragon.flickrImages!.first);
      return Image.network(
        dragon.flickrImages!.first,
        loadingBuilder: (_, __, ___) {
          return const Center(child: CircularProgressIndicator());
        },
        errorBuilder: (_, error, __) => Center(child: Text(error.toString())),
      );
    } else {
